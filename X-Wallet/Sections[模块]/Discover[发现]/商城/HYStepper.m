...
-(void)setupLayout{
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[_minusBtn(22)]" options:0 metrics:nil views:views]];
...
-(UITextField *)valueTF{
    if (!_valueTF) {
        ...
        tf.textColorPicker = IXColorPickerWithRGB(kBlackR,kWhiteR,kBlackR,kBlackR);
        _valueTF = tf;
    }
    return _valueTF;
}
...
-(UIButton *)minusBtn{
    if (!_minusBtn) {
        UIButton *btn = [self actionButtonWithTitle:@"main_sub_1"];
...
-(UIButton *)plusBtn{
    if (!_plusBtn) {
        UIButton *btn = [self actionButtonWithTitle:@"main_add_1"];
        btn.translatesAutoresizingMaskIntoConstraints = false;
